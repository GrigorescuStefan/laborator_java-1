...
public class ManagerCursuri {
	...
	public String[] showCoursesByStudent(Student student) {
		String[] listacursuri = new String[cursuri.size()];
		int nr = 0;
		for (Curs c : cursuri) {
			if (c.cautaStudent(student)) {
				if (c.returneazaNota(student) < 5)
					listacursuri[nr++] = c.nume + " - Nota: " + c.returneazaNota(student) + " - Restanta";
				else
					listacursuri[nr++] = c.nume + " - Nota: " + c.returneazaNota(student);
			}
		}
		return listacursuri;
	}
	...
	public int reportAverageGradesOfStudent(Student student){
		int nr=0,s=0;
		for(Curs c:cursuri){
			if(c.studenti == student)
			{
				s=s+c.nota.get(c.studenti);
				nr++;
			}
		}
		return s/nr;
